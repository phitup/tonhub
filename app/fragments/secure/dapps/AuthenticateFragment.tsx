...
const SignStateLoader = memo((props: { session: string; endpoint: string }) => {
  ...
  useEffect(() => {
    ...
    backoff("authenticate", async () => {
      ...
      let currentState = await axios.get(
        "https://" + props.endpoint + "/connect/" + props.session
      );
      if (ended) {
        return;
      }
  ...
  const approve = useCallback(
    async (selectedAccount?: SelectedAccount) => {
      ...
      let toSign = beginCell()
        ...
        .storeMaybeRef(beginCell().storeBuffer(Buffer.from(endpoint)).endCell())
        .storeRef(
      ...
      await backoff("authenticate", async () => {
        ...
        await backoff("authenticate", async () => {
          await axios.post(
            "https://connect.tonhubapi.com/connect/grant",
            { key: props.session },
            { timeout: 5000 }
          );
      ...
      if (addExt && state.app?.extension) {
        ...
        let slice = Cell.fromBoc(
          Buffer.from(state.app?.extension, "base64")
        )[0].beginParse();
  ...
  return (
    <DappAuthComponent
      ...
      setAddExtension={
        state.type === "initing" && state.app?.extension ? setAddExt : undefined
      }
      addExtension={
        state.type === "initing" && state.app?.extension ? addExt : undefined
      }
      single
    />
  );
